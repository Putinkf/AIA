...
### 5) `OSError: [WinError 10057]` при отправке сообщений из GUI

Причина: GUI запущен без подключенного Agent (или соединение было потеряно), а IPC-сокет недоступен.

Решение:

1. Сначала запустите Agent:
...
3. Если Agent был перезапущен — подождите 1–2 секунды, GUI переподключится автоматически.

Теперь при офлайн-агенте GUI не падает, а показывает статус `Offline` и предупреждение в лог-панели.
